...
<section class="content-header">
    ...
    <?= Component::Breadcrumb([['url' => '', 'text' => 'Inicio'], ['url' => 'agencias', 'text' => 'Lista agencias'], ['text' => 'Crear agencia']]) ?>
</section>
<!-- Main content -->
<div class="container">
    <?=form_open('', ['class' => 'form-horizontal col-md-8', 'style' => 'margin-left: 15%']) ?>
    <hr style="border: 1px solid #3D8EBC;"/>
    <?= $this->view("Agencias/FormAgencia", ["view" => "create"], true) ?>
    <?=  form_close() ?>
</div>
<?= $this->Footer() ?>
...
<script>
    ...
    function Save() {
        $.ajax({
            type: 'post',
            url: '<?= site_url('agencias/crearAgencia') ?>',
            data: $('form').serialize(),
            beforeSend: function () {
                $('body').addClass('Wait');
